...
class AccountsPool:
    ...
    async def set_active(self, username: str, active: bool | int, error_msg: str = None):
        qs = """
        UPDATE accounts SET 
    ...
    async def _get_and_lock(self, queue: str, condition: str, params: dict = None):
        ...
        if queue in self.endpoint_to_spread:
            ...
            lock_until = 120  # 2 minutes in seconds
        ...
        while True:
            ...
            if not account:
                ...
                if await self._account_needs_humanization(account=account,
                    ...
                    humanization_status: HTIOutput = await self.humanize_account(account=account, queue=queue)
                    login_status: int = humanization_status.login_status
                    if login_status != 1 or humanization_status.cookies is None:
                        return None
    ...
    async def set_cookies(self, username: str, cookies: dict | str, force_cookie: bool = False) -> None:
        # check cookie viability
        if isinstance(cookies, str):
            cookies = json.loads(cookies)
        auth_in_cookie: bool = ("ct0" in cookies.keys()) and ("auth_token" in cookies.keys())

        if not auth_in_cookie:
            logger.warning(f"{username}: cookies don't have the required keys \n\t- ct0 -> {'ct0' in cookies.keys()}\n\t- auth_token -> {'auth_token' in cookies.keys()}\n\t- force_cookie -> {force_cookie}")

        if auth_in_cookie or force_cookie:
    ...
    async def humanize_account(self, account: Account, queue: str) -> HTIOutput:
        """
        ...
        :rtype: int
        """
        from app.scraper.hti.humanTwitterInteraction import humanize
        ...
        if result.cookies is not None:  # update the cookies if not None
            await self.set_cookies(username=result.username, cookies=result.cookies)

        # log results
        status = "Successful" if result.login_status == 1 else "Failed"
        logger_fn = logger.info if result.login_status == 1 else logger.warning
        logger_fn(
            f"{status} humanization of account {account.username} on queue {queue} with activities {result.activities}")

        # update login information
        account.last_login = result.login_status
        account.num_calls = 0
        await self.save(account)
        return result
