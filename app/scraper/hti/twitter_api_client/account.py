...
class Account:
    ...
    def __init__(self, email: str = None, username: str = None, password: str = None, session: AsyncClient = None, **kwargs):
        ...
        self.session: AsyncClient = event_loop.run_until_complete(self._validate_session(email, username, password, session, **kwargs))
        self.rate_limits = {}
    ...
    async def v1(self, path: str, params: dict) -> dict:
        ...
        r = await self.session.post(
            ...
            data=params # TODO: check if this is okay
        )
        if self.debug:
            log(self.logger, self.debug, r)
        return r.json()
    ...
    async def create_poll(self, text: str, choices: list[str], poll_duration: int) -> dict:
        options = {
            "twitter_api_client:card": "poll4choice_text_only",
            "twitter_api_client:api:api:endpoint": "1",
            "twitter_api_client:long:duration_minutes": poll_duration  # max: 10080
        }
        for i, c in enumerate(choices):
            options[f"twitter_api_client:string:choice{i + 1}_label"] = c

        headers = get_headers(self.session)
        headers['content-type'] = 'application/x-www-form-urlencoded'
        url = 'https://caps.twitter.com/v2/cards/create.json'
        r = await self.session.post(url, headers=headers, params={'card_data': orjson.dumps(options).decode()})
    ...
    async def add_list_member(self, list_id: int, user_id: int) -> dict:
        return await self.gql('POST', Operation.ListAddMember, {'listId': list_id, "userId": user_id})

    async def remove_list_member(self, list_id: int, user_id: int) -> dict:
        return await self.gql('POST', Operation.ListRemoveMember, {'listId': list_id, "userId": user_id})
    ...
    async def pin(self, tweet_id: int) -> dict:
        return await self.v1('account/pin_tweet.json', {'tweet_mode': 'extended', 'id': tweet_id})

    async def unpin(self, tweet_id: int) -> dict:
        return await self.v1('account/unpin_tweet.json', {'tweet_mode': 'extended', 'id': tweet_id})
    ...
    async def update_profile_image(self, media: str) -> Response:
        ...
        r = await self.session.post(url, headers=headers, params=params)
        return r
    ...
    async def remove_interests(self, *args):
        """
    ...
    async def home_timeline(self, limit=math.inf) -> list[dict]:
        return await self._paginate('POST', Operation.HomeTimeline, Operation.default_variables, limit)

    async def home_latest_timeline(self, limit=math.inf) -> list[dict]:
        return await self._paginate('POST', Operation.HomeLatestTimeline, Operation.default_variables, limit)

    async def bookmarks(self, limit=math.inf) -> list[dict]:
        return await self._paginate('GET', Operation.Bookmarks, {}, limit)

    async def _paginate(self, method: str, operation: tuple, variables: dict, limit: int) -> list[dict]:
    ...
    async def _upload_media(self, filename: str, is_dm: bool = False, is_profile=False) -> int | None:
        ...
        r = await self.session.post(url=url, headers=headers, params=params)
            ...
            with open(file, 'rb') as fp:
                ...
                while chunk := fp.read(UPLOAD_CHUNK_SIZE):
                    ...
                    try:
                        ...
                        r = await self.session.post(url=url, headers=headers | _headers, params=params, content=data)
                    except Exception as e:
                        if self.debug:
                            self.logger.error(f'Failed to upload chunk, trying alternative method\n{e}')
                        try:
                            files = {'media': chunk}
                            r = await self.session.post(url=url, headers=headers, params=params, files=files)
                        except Exception as e:
                            if self.debug:
        ...
        while processing_info:
            ...
            r = await self.session.get(url=url, headers=headers, params=params)
            processing_info = r.json().get('processing_info')
    ...
    @staticmethod
    async def _validate_session(*args, **kwargs) -> AsyncClient:
        ...
        if isinstance(cookies, str):
            _session = AsyncClient(cookies=orjson.loads(Path(cookies).read_bytes()), follow_redirects=True)
            _session._init_with_cookies = True
            _session.headers.update(get_headers(_session))
            return _session
    ...
    async def dm_history(self, conversation_ids: list[str] = None) -> list[dict]:
        ...
        if conversation_ids:
            ...
            inbox = await self.dm_inbox()
            ids = list(inbox['inbox_initial_state']['conversations'])
        event_loop = asyncio.get_event_loop()
        return event_loop.run_until_complete(process(ids))
    ...
    async def dm_delete(self, *, conversation_id: str = None, message_id: str = None) -> dict:
        ...
        if conversation_id:
            results['conversation'] = (await self.session.post(
                f'{self.v1_api}/dm/conversation/{conversation_id}/delete.json')).text  # not json response
        if message_id:
            # delete single message
            _id, op = Operation.DMMessageDeleteMutation
            results['message'] = (await self.session.post(
                f'{self.gql_api}/{_id}/{op}',
                json={'queryId': _id, 'variables': {'messageId': message_id}})).json()
        return results
    ...
    async def clear_scheduled_tweets(self) -> None:
        ...
        for _id in set(find_key(drafts, 'rest_id')):
            if _id != user_id:
                await self.gql('POST', Operation.DeleteScheduledTweet, {'scheduled_tweet_id': _id})
    ...
    async def notifications(self, params: dict = None) -> dict:
        r = await self.session.get(
    ...
    async def recommendations(self, params: dict = None) -> dict:
        r = await self.session.get(
    ...
    async def fleetline(self, params: dict = None) -> dict:
        r = await self.session.get(
