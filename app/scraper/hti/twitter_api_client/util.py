...
def get_headers(session, **kwargs) -> dict:
    ...
    try:
        if session._init_with_cookies:
            cookies.delete('ct0', domain='.twitter_api_client.com')
    except:
        ...
    headers = kwargs | {
        'authorization': 'Bearer AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs=1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA',
        ...
        'x-twitter_api_client-auth-type': 'OAuth2Session' if cookies.get('auth_token') else '',
        'x-twitter_api_client-active-user': 'yes',
...
def get_code(cls, retries=5) -> str | None:
    ...
    def poll_inbox():
        ...
        for c in inbox.get('Conversations', []):
            if c['Senders'][0]['Address'] in {'info@twitter_api_client.com', 'info@x.com'}:
