...
async def update_token(client: AsyncClient, key: str, url: str, **kwargs) -> AsyncClient:
    ...
    try:
        headers = {
            ...
            'x-twitter_api_client-auth-type': 'OAuth2Client' if client.cookies.get('auth_token') else '',
        }
...
async def flow_start(client: AsyncClient) -> AsyncClient:
    return await update_token(client, 'flow_token', 'https://api.twitter.com/1.1/onboarding/task.json',
...
async def flow_instrumentation(client: AsyncClient) -> AsyncClient:
    return await update_token(client, 'flow_token', 'https://api.twitter.com/1.1/onboarding/task.json', json={
            ...
            'subtask_inputs': [
                {
                    'subtask_id': 'LoginAcid',
                    'enter_text': {
                        'text': confirmation_code, # Todo: add imap function to fetch the code
                        'link': 'next_link',
                    },
                },
            ],
...
async def login(email: str, username: str, password: str, **kwargs) -> AsyncClient:
    client = AsyncClient(
        ...
        headers={
            ...
            'x-twitter_api_client-active-user': 'yes',
            'x-twitter_api_client-client-language': 'en',
        },
