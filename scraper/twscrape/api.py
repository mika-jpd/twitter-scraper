...
class API:
    ...
    def __init__(
        ...
        self._num_calls_before_humanization: tuple[int, int] = (15, 30)
    ...
    async def _gql_item(self, op: str, kv: dict, ft: dict | None = None):
        ft = ft or {}
        queue = op.split("/")[-1]
        async with QueueClient(self.pool, queue, self.debug, proxy=self.proxy, use_case=self.use_case) as client:
