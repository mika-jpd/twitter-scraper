...
from scraper.hti.scraping_utils.email_utils import get_verification_code, get_verification_code_imap, get_authentication_code, get_authentication_code_imap
import asyncio
import random
import httpx
import numpy as np
from typing import Awaitable, Any
from zendriver import Tab, Element
from scraper.hti.scraping_utils.typer import Typer
...
class TwitterActions:
    ...
    async def login_manually(self,
        ...
        if verification_code:
            await self.unlock_by_email_verification()
    ...
    async def retweet(self, name='retweet', num_retweets: int = 2, progress: dict = None) -> bool:
        ...
        timeline_tweets_rt: list[Element] = await self.tab.select_all("div[aria-label='Timeline: Your Home Timeline'] div[data-testid='cellInnerDiv'] button[data-testid='retweet']")
        tweet_rt_buttons: list[Element] = random.sample(timeline_tweets_rt, k=min(num_retweets, len(timeline_tweets_rt)))
    ...
    async def view_trending(self, name="view_trending", progress: dict = None) -> bool:
        ...
        if np.random.randint(0, 10) > 5:
            ...
            butn = await self.tab.select('a[href="/explore/tabs/keyword"]')
            await butn.click()
            await asyncio.sleep(np.random.randint(1, 20) / 20)
