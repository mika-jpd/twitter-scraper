...
async def get_authentication_code(email_address: str, service=None) -> str:
    ...
    query = f"from:info_at_x.com_{email_address}@duck.com"
    code = None
    for i in range(10):
...
async def get_verification_code(email_address: str, service=None) -> str:
    ...
    # after authentication
    # search for the confirmation email
    query = f"from:verify_at_x.com_{email_address}@duck.com"
    code = None
    for i in range(10):
        emails = search_messages(service=service, query=query)
        if code:
...
if __name__ == '__main__':
    code = asyncio.run(get_verification_code_imap(
        email_address='hedeqvgf@esponamail.com',
        password='xifjuqaoA!2399'
    ))
    print(code)"""